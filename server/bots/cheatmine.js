...
export class CMBot extends MainBot {
  ...
  processJoinMessage() {
    ...
    const matchJoin = this.textMessage.match(/\[\*]\s*(.*?)\s*присоеденился/);
  ...
  processLeaveMessage() {
    ...
    const matchLeave = this.textMessage.match(/\[\*]\s*(.*?)\s*покинул/);
  ...
  processCommandMessage() {
    ...
    const matchCmd = this.textMessage.match(/]\s*(.*?)\s*->.*?]\s*(.*)/);
    if (matchCmd?.[1]) {
