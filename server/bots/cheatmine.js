...
export class CMBot extends EventEmitter {
  constructor(options) {
    super();
    ...
    this.host = "ru.cheatmine.net";
    this.curServer = "cheatmine";
    this.lastUser = "";
    this.currentArg = "";
    this.allArgs = [];
  ...
  reconnectBot(reason) {
    ...
    saveDeaths();
    delete this;
    botsObj[this.curServer][this.portal] = new CMBot(this.options);
  }
  ...
  sendAdvertisements() {
    setInterval(() => this.sendMsg(adMsgs[randInt(0, adMsgs.length - 1)]), randInt(60*1000, 1.5*60*1000));
  ...
  messagesMonitoring(message) {
    ...
    else if (textMessage.startsWith("[ʟ]") || textMessage.startsWith("[ɢ]")) {
      try {
        username = message.json.extra[0].clickEvent.value.split(" ")[1];
      } catch (err) {
        username = "undefined";
    ...
    const matchLeave = textMessage.match(/\[\*]\s*(.*?)\s*покинул/);
    const matchJoin = textMessage.match(/\[\*]\s*(.*?)\s*присоеденился/);
    const matchCmd = textMessage.match(/]\s*(.*?)\s*->.*?]\s*(.*)/);
