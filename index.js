...
const adMsgs = [
  "!&c&lПривет, друг! Хочешь побывать в клане, где была великая история? Тогда тебе сюда -> /warp CH ! У нас есть: топовый кит для пвп, хороший кх и многое другое! Чего же ты ждёшь? Присоединяйся к нам!",
  "!&c&lПриветик! Хочешь с кайфом провести время, но не знаешь как? Тогда тебе подойдёт клан &4&lChert&0&lHouse &c&l! У нас ты найдёшь хороший кх, топовый кит и уважение клана. Чтоб вступить в клан пиши /warp CH",
  "!&c&lХочешь в крутой клан с многими плюшками? Тогда тебе нужен клан &4&lChert&0&lHouse&c&l ! У нас ты не только найдёшь топовый кит для пвп и хороший кх, но и дс сервер! А так же у нас открыт набор на модераторов! /warp CH",
  "!&c&lИщешь отличный клан с крутыми возможностями? Тогда тебе подходит клан &4&lChert&0&lHouse &c&l! Топовый кит, функциональный бот, всё это ты найдёшь на /warp CH. Заинтересовало? Ждём именно тебя!",
];
const unterMsgs = {
...
function sendAdvertisements(bot, ourText) {
  setInterval(() => {
    sendMsg(bot,"/gm 1");
    // bot.unequip("head");
    sendMsg(bot,"/tptoggle disable");
    sendMsg(bot,"/clear");
    if (ourText === uhaText) sendMsg(bot, ourText);
    else sendMsg(bot, adMsgs[getRandomNumber(0, adMsgs.length - 1)]);
...
function createBot(nickname, portal, warp = allBotWarp, our = true, chatWriting = false, password = defPassword, host = "mc.masedworld.net", port = 25565, auth = "offline") {
  ...
  if (our) sendAdvertisements(bot, "svoi");
...
createBot("Temper1ng", "s2", "n71kfht1", false);
